...
public class RequestInfo {
    ...
    private MetadataWithMergedTags metadata;
    ...
    private RequestType determineRequestType(HttpServletRequest request) {
        MediaTypeUtil.ReturnType returnType = MediaTypeUtil.parseMediaType(request.getHeader("Accept"));
    ...
    private void determineRequestedMetrics(String uri) {
        ...
        if (splittedUri.length > 2 && getSingleRequestedRegistry() != null) {
            ...
            if (requestType.equals(RequestType.JSON_METADATA)) {
                metadata = ((MetricRegistryImpl) getSingleRequestedRegistry()).getMetadataWithTags().get(metricName);
                if (metadata == null) {
                    metricsRequested = MetricsRequested.NOT_FOUND;
                    return;
                }
                ...
                for (Map.Entry<MetricID, Metric> entry : getSingleRequestedRegistry().getMetrics().entrySet()) {
                    if (entry.getKey().getName().equals(metricName)) {
                        metricsCollection.getMetrics().put(entry.getKey(), entry.getValue());
                    }
                }
    ...
    public MetadataWithMergedTags getMetadata() {
        return metadata;
    }
